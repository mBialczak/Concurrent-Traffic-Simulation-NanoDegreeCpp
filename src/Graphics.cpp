...
void Graphics::loadBackgroundImg()
{
  ...
  // second element will be the transparent overlay
  _images.push_back(background.clone());
  // third element will be the result image for display
  _images.push_back(background.clone());
}
  ...
  for (auto it : _trafficObjects) {
    ...
    if (it->getType() == ObjectType::objectIntersection) {
      ...
      cv::Scalar trafficLightColor = intersection->trafficLightIsGreen() == true
          ...
          : cv::Scalar(0, 0, 255);
      cv::circle(
          _images.at(1), cv::Point2d(posx, posy), 25, trafficLightColor, -1);
